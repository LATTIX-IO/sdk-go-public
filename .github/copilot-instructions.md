...
Source: LATTIX-IO/lattix-monorepo:.github/copilot-instructions.md
Bundle: 2026.05.05
...
`sdk-go-public` is a submodule of **lattix-monorepo** and shares the Nexus Obsidian-backed memory system (`50-Memory/`). This block is generated by `bootstrap_repos`.

- **Stack (detected):** Go
- **Build / test (detected):** `go build ./...` · `go test ./...`
- **Scope:** `project: lattix`. Memory about this repo is `scope: repo`, `repo: sdk-go-public`; cross-repo/architecture memory belongs in the monorepo (`scope: project`).
